...
使用软件的**打开项目**按钮，选择并打开``{Spring Boot项目路径}/main/expands/SpringGenerate/project.sg.json``文件。

![](./readme-images/edit-table-1.png)

项目打开时，如上图所示。

整个页面使用两个分割线分为了三部分，最上方的部分是**标题栏**，左侧部分是**数据结构列表**，右侧部分是**表结构编辑器**。

* **标题栏**：

  ![](./readme-images/edit-table-2.png)

​	标题栏从左到右分别标注了“项目名称”，“项目构建模板”，“数据库引擎”和“ROM框架”。最右侧的返回按钮可以返回到欢迎页面。

* **数据结构列表**：

  ![](./readme-images/edit-table-3.png)

  这一部分列出了这个项目中，通过这个软件组织管理的数据结构名称，也可以在这里新建一个数据结构。

* **表结构编辑器**：

  表结构编辑器就是剩余的右侧的部分，稍后将为您演示如何使用。

#### （1）、新建表

本次将使用一个**UserProject**案例来完成介绍，所以我们在这里新建一个表，表结构取名为**User**。

点击**数据结构列表**的**新建**按钮，系统将会创建一个新的表结构，如图所示。

![](./readme-images/edit-table-4.png)

系统默认创建的表结构名是“未命名”的表结构，点击选中**未命名_1**，就可以看到右侧的表结构编辑器出现了表结构的内容。

![](./readme-images/edit-table-5.png)**发行日志**
